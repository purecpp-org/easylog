...
template <size_t N, char c> inline void to_int(int num, char *p, int &size) {
  for (int i = 0; i < N; i++) {
...
public:
  ...
  appender(const std::string &filename, bool async, bool enable_console,
      ...
      : has_init_(true), flush_every_time_(flush_every_time),
        enable_console_(enable_console), max_file_size_(max_file_size) {
    filename_ = filename;
    ...
    write_thd_ = std::thread([this] {
      while (!stop_) {
        ...
        if (queue_.size_approx() == 0) {
          ...
          cnd_.wait(lock, [&]() { return queue_.size_approx() > 0 || stop_; });
        }
  ...
  template <bool sync> auto &get_mutex() {
    if constexpr (sync) {
      return mtx_;
    } else {
      return empty_mtx_;
